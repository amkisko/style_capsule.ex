defmodule Mix.Tasks.StyleCapsule.Build do
  ...
  def run(_args) do
    ...
    specs_with_styles = Enum.filter(specs, fn spec ->
    ...
    file_cached_specs = Enum.filter(specs_with_styles, fn spec ->
      spec.cache_strategy == :file
    end)

    runtime_specs = Enum.filter(specs_with_styles, fn spec ->
      spec.cache_strategy != :file
    end)
    ...
    Mix.shell().info("Found #{map_size(specs_by_namespace)} namespace(s) with file-cached components: #{Enum.join(Enum.map(specs_by_namespace, fn {ns, _} -> inspect(ns) end), ", ")}")
            ...
            {entries, count, namespace_css} =
              Enum.reduce(namespace_specs, {[], 0, []}, fn spec, {acc, n, css_acc} ->
                ...
                if spec.cache_strategy != :file do
                  Mix.shell().warn("Skipping component #{inspect(spec.module)} with cache_strategy #{inspect(spec.cache_strategy)} - only :file components are precompiled")
                  {acc, n, css_acc}
                else
                  case build_component(spec, output_dir, fallback_dir) do
                    {:ok, entry} ->
                      ...
                      final_css = if StyleCapsule.Config.include_comments() do
        ...
        case File.write(namespace_path, namespace_css_content) do
          :ok ->
            Mix.shell().info("Generated CSS for namespace #{inspect(namespace)}: #{namespace_path} (#{count} component(s))")
    ...
    runtime_entries = Enum.map(runtime_specs, fn spec ->
      %{
        ...
        path: nil  # Runtime components don't have file paths
      }
    end)
    ...
    build_metadata = %{
      ...
      namespaces: Enum.map(namespace_files, fn {ns, path, _, count} -> %{namespace: ns, file: path, component_count: count} end),
      entries: all_entries,
    ...
    Mix.shell().info("  - #{total_file_cached} precompiled (cache_strategy: :file) across #{map_size(specs_by_namespace)} namespace(s)")
          ...
          case Code.ensure_loaded(module) do
            {:module, _} ->
              ...
              if function_exported?(module, :style_capsule_spec, 0) do
                try do
                  spec = apply(module, :style_capsule_spec, [])
